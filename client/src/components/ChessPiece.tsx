...
// Cburnett-style classic SVG pieces. viewBox 0 0 45 45.
// Colors are injected at render time so a single definition covers both sides.
const PATHS: Record<PieceType, (fill: string, stroke: string) => string> = {
  K: (f, s) =>
...
export function ChessPiece({ piece, color, size }: Props) {
  const fill   = color === 'w' ? '#f5f5f0' : '#1a1a1a';
  const stroke = color === 'w' ? '#1a1a1a' : '#f0f0eb';
  const inner  = PATHS[piece]?.(fill, stroke) ?? '';
