...
export type PieceSet = {
  key: string;
  label: string;
  // ds = "detail stroke": when provided, interior line-art paths use ds instead of stroke.
  // Pass stroke='none' + ds=white for black pieces so only interior lines get the light outline.
  paths: Record<PieceType, (fill: string, stroke: string, ds?: string) => string>;
};
...
const cburnettPaths: PieceSet['paths'] = {
  K: (f, s, ds) =>
    ...
    `<path d="M22.5 11.6V6m-3 0h6" stroke="${ds ?? s}" stroke-linecap="round"/>` +
    ...
    `<path d="M11.5 30c5.5-3 15.5-3 21 0m-21 3.5c5.5-3 15.5-3 21 0m-21 3.5c5.5-3 15.5-3 21 0" stroke="${ds ?? s}"/>` +
    `</g>`,
  ...
  B: (f, s, ds) =>
    ...
    `</g>` +
    `<path d="M17.5 26h10M15 30h15m-7.5-14.5v5M20 18h5" stroke="${ds ?? s}" stroke-linecap="round"/>` +
    `</g>`,
  ...
  N: (f, s, ds) =>
    ...
    `<path d="M9.5 25.5a.5.5 0 1 1-1 0 .5.5 0 1 1 1 0z" fill="${ds ?? s}"/>` +
    `<path d="M14.933 15.75a.5 1.5 30 1 1-.866-.5.5 1.5 30 1 1 .866.5z" fill="${ds ?? s}"/>` +
    `</g>`,
...
const alphaPaths: PieceSet['paths'] = {
  K: (f, s, ds) =>
    ...
    `<path d="M22.5 7.5v9M18.5 12h8" stroke="${ds ?? s}" stroke-width="2.2"/>` +
...
const meridaPaths: PieceSet['paths'] = {
  K: (f, s, ds) =>
    ...
    `<path d="M22.5 11V5.5m-3.5 0h7" stroke="${ds ?? s}" stroke-linecap="round"/>` +
    ...
    `<path d="M11 30c6-3.5 17-3.5 22.5 0m-22.5 4c6-3.5 17-3.5 22.5 0" stroke="${ds ?? s}"/>` +
    `</g>`,
